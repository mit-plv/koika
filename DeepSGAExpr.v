...
Section ActionValue.
  ...
  | avif: forall {T}, Val bool -> ActionValue T -> ActionValue T -> ActionValue T
  | avbind: forall {T T'}, ActionValue T -> (var T -> ActionValue T') -> ActionValue T'
  | avreturn: forall {T}, Val T -> ActionValue T
  | avwrite : forall (vec  port:nat) (idx: Val nat), Val (types vec) -> ActionValue unit
  | avblock : ActionValue unit.
...
Example term1 {var} {ctx}: ActionValue var ctx unit :=
  (avbind (avreturn (avread 0 0 (avinteger 0)))
          (fun x =>
