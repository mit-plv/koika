...
Module External <: External_sig.
  ...
  Definition Sigma (fn: ext_fn_t) :=
    match fn with
    | ext_mem imem => {$ struct_t cache_mem_input ~> struct_t cache_mem_output $}
    | ext_mem dmem => {$ struct_t cache_mem_input ~> struct_t cache_mem_output $}
    | ext_mem mainmem => {$ struct_t mem_input ~> struct_t mem_output $}
    | ext_uart_read => {$ bits_t 1 ~> uart_output $}
    | ext_uart_write => {$ uart_input ~> bits_t 1 $}
    | ext_led => {$ led_input ~> bits_t 1 $}
    end.
