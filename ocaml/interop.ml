open Cuttlebone

let writeout fname fn circuit =
  Common.with_output_to_file fname (fun out -> fn out circuit)

let fname ?directory (pkg: _ Extr.koika_package_t) ext =
  let modname = Util.string_of_coq_string pkg.koika_module_name in
  let dirname = match directory with Some d -> [d] | None -> [] in
  let fname = Printf.sprintf "%s%s" modname ext in
  String.concat "/" (dirname @ [fname])

let compile_simulation ?directory (kp: _ Cuttlebone.Extr.koika_package_t) (sp: _ Extr.sim_package_t) =
  Backends.Cpp.main (fname ?directory kp "") `Exe (Backends.Cpp.input_of_sim_package kp sp)

let compile_circuits ?directory (kp: _ Cuttlebone.Extr.koika_package_t) (vp: _ Extr.verilog_package_t) =
  let circuit = Graphs.graph_of_verilog_package kp vp in
  writeout (fname ?directory kp ".dot") Backends.Dot.main circuit;
  writeout (fname ?directory kp ".v") Backends.Verilog.main circuit

let compile_all ?directory
      (kp: _ Cuttlebone.Extr.koika_package_t)
      (sp: _ Extr.sim_package_t)
      (vp: _ Extr.verilog_package_t) =
  compile_simulation ?directory kp sp;
  compile_circuits ?directory kp vp
