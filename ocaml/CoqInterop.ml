...
let fname ?directory (pkg: _ SGA.sga_package_t) ext =
...
let compile_simulation ?directory (sga: _ SGALib.SGA.sga_package_t) (sp: _ SGA.sim_package_t) =
  Backends.Cpp.main (fname ?directory sga "") `Exe (Backends.Cpp.input_of_sim_package sga sp)

let compile_circuits ?directory (sga: _ SGALib.SGA.sga_package_t) (vp: _ SGA.verilog_package_t) =
...
let compile_all ?directory
      (sga: _ SGALib.SGA.sga_package_t)
      (sp: _ SGA.sim_package_t)
      (vp: _ SGA.verilog_package_t) =
