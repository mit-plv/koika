...
Notation "'let' a ':=' b 'in' c" := (UBind a b c) (in custom koika at level 91, a custom koika_var at level 0, right associativity, format "'[v' 'let'  a  ':='  b  'in' '/' c ']'").
...
Notation "'->' instance '.(' method ')' '(' args ')'" :=
  (USugar (UCallModule instance id method args))
    (in custom koika at level 0, instance constr at level 0, method constr at level 0, args custom koika_args at level 0).
...
Notation "f ':=' expr" := (cons (f,expr) nil) (in custom koika_structs_init at level 20, f custom koika_var at level 0, expr custom koika at level 89).
...
Module Type Tests.
  ...
  Definition test_30'' : uaction reg_t :=
    {{
        struct mem_req {| foo := upu[#(Bits.of_nat 3 0) :+ 2] ; bar := magic |}
    }}.
