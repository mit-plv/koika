...
Section Ex.
  ...
  Notation scheduler := (scheduler TFn).
  Notation circuit := (circuit TFn).

  Example testprog0 : rule string TFn :=
    (Write P0 0 (Const [true; true])).
  (* Example testprog1 : rule string TFn := *)
  (*   (If (Read P0 0) *)
  ...
  Check (compile_rule env_nil testprog (adapter {| sregs := Vector.map init_rwdata [CExternal (ReadReg 0) []] |})).
  ...
  Definition compiled_example : circuits 2 :=
    must (compile_scheduler (Try testprog Done Done) [CExternal (ReadReg 0) []; CExternal (ReadReg 1) []]).

  Definition compiled_example_ls : list circuit :=
    Vector.to_list compiled_example.

  Definition interp_TFn ext (args: list bits) : option bits :=
      match ext, args with
  ...
  Compute (interp_circuits interp_TFn _ compiled_example).
  ...
  (*                                    Some (interp_circuits interp_TFn _ example))) = None. *)
  (* Proof. *)
